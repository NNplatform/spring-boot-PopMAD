...
public class ProductServiceClient {
    ...
    @CircuitBreaker(name = "productService", fallbackMethod = "getWishListFallback")
    public JsonResponse getWishList() {
        String url = productServiceUrl + "/wishlist";
        ResponseEntity<String> responseEntity = restTemplate.exchange(
                url,
                HttpMethod.GET,
                null,
    ...
    private ResponseEntity<JsonResponse> getWishListFallback(Throwable t) {
        JsonResponse response = new JsonResponse<>("-1", "Error fetching all products", t.getMessage(), Collections.emptyList());
        return ResponseEntity.ok(response);
    }
