...
public class GlobalExceptionHandler {
    ...
    @ResponseBody
    public ResponseEntity<JsonResponse<?>> handleCartIdNotFoundException(CartIdNotFoundException ex) {
        JsonResponse<?> jsonResponse = new JsonResponse<>(ex.getCode(), ex.getMessage());
        return ResponseEntity.status(HttpStatus.OK).body(jsonResponse);
    }

    @ExceptionHandler(CartItemIdNotFoundException.class)
    @ResponseBody
    public ResponseEntity<JsonResponse<?>> handleCartItemIdNotFoundException(CartItemIdNotFoundException ex) {
        JsonResponse<?> jsonResponse = new JsonResponse<>(ex.getCode(), ex.getMessage());
        return ResponseEntity.status(HttpStatus.OK).body(jsonResponse);
    }
